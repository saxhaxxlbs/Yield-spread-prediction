...
final_data_long <- final_data %>%
  ...
  select(date, inflation_expectation, cpi_yoy, pce_yoy, spread) %>%
  pivot_longer(
    cols = c(inflation_expectation, cpi_yoy, pce_yoy),  # Columns to transform into rows
...
ggplot(final_data_long, aes(x = date)) +
  # Highlight specific periods with transparent grey rectangles to emphasize periods of interest
  geom_rect(
    aes(xmin = as.Date("1984-01-01"), xmax = as.Date("1987-01-01"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +
  geom_rect(
    aes(xmin = as.Date("1988-04-01"), xmax = as.Date("1991-10-01"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +
  geom_rect(
    aes(xmin = as.Date("1998-04-01"), xmax = as.Date("1999-01-01"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +
  geom_rect(
    aes(xmin = as.Date("2000-02-01"), xmax = as.Date("2002-06-01"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +
  geom_rect(
    aes(xmin = as.Date("2002-01-01"), xmax = as.Date("2006-08-01"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +
  geom_rect(
    aes(xmin = as.Date("2008-10-01"), xmax = as.Date("2009-08-01"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +
  geom_rect(
    aes(xmin = as.Date("2016-09-01"), xmax = as.Date("2020-02-01"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +
  geom_rect(
    aes(xmin = as.Date("2021-03-01"), xmax = as.Date("2024-10-15"), ymin = -Inf, ymax = Inf),
    fill = "#EEEDED", alpha = 0.05, inherit.aes = FALSE
  ) +

  # Add stacked bar chart for inflation measures, grouped by `inflation_measure`
  geom_bar(
    aes(y = value, fill = inflation_measure),  # Map bar heights to `value` and fill to `inflation_measure`
    stat = "identity",  # Use actual data values for bar heights
    position = "stack",  # Stack the bars for cumulative representation
    alpha = 0.7,  # Add transparency for better visualization
    width = 1  # Remove gaps between bars by maximizing their width
  ) +

  # Add a line for `spread` from a different dataset for comparison
  geom_line(data = final_data, 
    aes(y = spread, color = "Spread"), size = 1  # Map y-values to `spread` and set line thickness
  ) +

  # Add titles and labels for clarity and context
  labs(
    x = "",  # No label for x-axis to maintain visual simplicity
    y = "",  # No label for y-axis to reduce clutter
    title = "Inflation and Spread have negative, yet not strong, relationship",  # Clear main title
    subtitle = "Highlighted areas show periods of negative correlation"  
  ) +

  # Customize bar and line colors to differentiate inflation measures and spread
  scale_fill_manual(
    values = c(
      "cpi_yoy" = "#d6604d",  # Dark red for CPI YoY
      "inflation_expectation" = "#f4a582",  # Medium orange for Inflation Expectations
      "pce_yoy" = "#fddbc7"  # Light pink for PCE YoY
    ),
    labels = c(
      "CPI YoY",  # CPI Label
      "Inflation Expectations",  # Inflation Expectations Label
      "PCE YoY"  # PCE Label
    )
  ) +
  scale_color_manual(
    values = c("Spread" = "#67001f"),  # Dark maroon for the Spread line
    labels = c("Spread")  # Label for the Spread line
  ) +

  # Adjust x-axis to show every 2 years and restrict the range to match the data
  scale_x_date(
    date_breaks = "2 year",  # Show ticks every 2 years
    date_labels = "%Y",  # Use year-only labels
    limits = c(as.Date("1984-01-01"), as.Date("2024-11-30"))  # Restrict the axis range to the data
  ) +

  # Format y-axis to display percentages
  scale_y_continuous(labels = percent_format(scale = 1)) +

  # Apply a clean and professional black-and-white theme
  theme_bw(base_family = "montserrat") +
  theme(
    legend.position = "bottom",  # Position legend below the plot for better layout
    ...
    plot.title = element_text(size = 18, face = "bold", hjust = 0, color = "black"),  # Bold and prominent main title
    plot.subtitle = element_text(size = 16, hjust = 0, color = "black"),  # Slightly smaller subtitle
    axis.text.x = element_text(angle = 0, vjust = 0.5, size = 10),  # Horizontal x-axis labels
    axis.text.y = element_text(size = 10)  # Maintain uniform y-axis label size
  )
...
ggplot(final_data, aes(x = vacancy_unemployed_ratio, y = spread)) +
  ...
  # Add a shaded horizontal line at y = 0
  ...
  # Customize the y-axis to start at -1.5
  scale_y_continuous(
    limits = c(-1.5, 3),  # Set y-axis limits from -1.5 to 3
    breaks = seq(-1.5, 3, 0.5)  # Add ticks every 0.5 units
  ) +
  
  # Add an annotation to label the VU ratio with a custom position and styling
  annotate("text", x = 1, y = -1.7, label = "VU ratio", size = 4.5, hjust = 0.5, color = "black") +
...
# Add percentage column to `cor_spread` for labeling
cor_spread <- cor_spread %>%
  mutate(Percentage = round(abs(Correlation) * 100, 1))  # Convert correlation to percentage

# Generate the correlation plot with percentages
ggplot(cor_spread, aes(x = Correlation, y = Variable, fill = Variable)) +
  ...
  geom_text(aes(
    label = round(Correlation, 2),  # Format as percentage
    x = Correlation + sign(Correlation) * 0.02  # Offset text slightly outside the bar
  ),
  hjust = ifelse(cor_spread$Correlation > 0, -0.2, 1.2),  # Align text based on direction of bar
  size = 4,
  fontface = "bold",  # Make text bold for better visibility
  color = "black"  # Use consistent text color
  ) +  # Text size
...
## Can the model inform spread trading strategy?
...
ggplot(test_data, aes(x = date)) +
  ...
  labs(
    ...
    caption = "\nDisclaimer: This analysis is for informational purposes only and does not constitute financial advice or a recommendation to undertake any specific trading strategy."  # Disclaimer text
  ) +
